...
public class EntropyCalculation {
	...
	public Node maxGainedElement(List<List<Double>> feature, List<Double> label) {	
		...
		for(int i = 0; i < feature.get(0).size(); i++) {
			
			List<Double> singleFeature = new ArrayList<>(); //new

			for(int j = 0; j < feature.size(); j++) {

				//System.out.println("Single feature: " + subset.attribute.get(j).get(i));
				
				singleFeature.add(feature.get(j).get(i));
			}		
